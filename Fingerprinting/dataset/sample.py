...
def sliding_window(data_dir, datatype, window_size, sample_ratio=1):
    ...
    labels = []
    if datatype == 'train':
        data_dir += 'dataset/processing_raw_data/final_output/'
    if datatype == 'val':
        data_dir += 'hdfs/test_normal'
    ...
    for i in range (df.shape[0]):
        result_logs.append(df.iloc[i:i+window_size,:])
        labels.append()
    with open(data_dir, 'r') as f:
        for line in f.readlines():
            ...
            line = tuple(
                map(
                    lambda n: n - 1, map(int, line.strip().split())
                )
            )
            ...
            for i in range(len(line) - window_size):
                ...
                labels.append(line[i + window_size]) #the next event after the window
